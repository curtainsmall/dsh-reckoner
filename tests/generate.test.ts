...
const ROWS: TraceRow[] = [
  row(1, TraceTool.Start, { title: 'What is P?', record: '1' }),
  row(2, TraceTool.Set, { name: 'V', value: { num: 12, dim: [2, 1, -3, -1, 0, 0, 0] } }),
  row(3, TraceTool.Set, { name: 'R', value: { num: 100, dim: [2, 1, -3, -2, 0, 0, 0] } }),
  row(4, TraceTool.Message, { text: 'Ohm law and P = V*I' }),
  row(5, TraceTool.Message, { text: 'state the result in watt', hide: true }),
  row(6, TraceTool.Eval, { formula: '@V/@R', target: 'I', rev: 1, vars: { V: 12, R: 100 }, result: { num: 0.12, dim: [0, 0, 0, 1, 0, 0, 0] } }),
  row(7, TraceTool.Eval, { formula: '@V/@R', target: 'I2', rev: 1, vars: {}, result: null }, false),
  row(8, TraceTool.Get, { name: 'I', value: { num: 0.12 } }),
  row(9, TraceTool.Set, { name: 'R', value: null }),
  row(10, TraceTool.End, { text: 'P = 1.44 W', record: '1' }),
]
...
describe('article facts', () => {
  it('takes the title from the record_start row', () => {
    ...
    expect(recordFacts([row(1, TraceTool.Start, { record: '1' })]).title).toBe('')
    expect(recordFacts([row(1, TraceTool.Start, { title: 'refused', record: '1' }, false)]).title).toBe('')
  })
  ...
  it('skips a refused message', () => {
    expect(recordFacts([row(1, TraceTool.Message, { text: 'refused' }, false)]).messages).toEqual([])
  ...
  it('keeps only the successful eval rows as steps', () => {
    ...
    expect(recordFacts([row(1, TraceTool.Eval, { formula: '@V/@R', target: 'I' })]).steps).toEqual([
  ...
  it('takes the closing text from the record_end row', () => {
    ...
    expect(recordFacts([row(1, TraceTool.End, { record: '1' })]).closing).toBeNull()
  ...
  it('keeps the last write of a rewritten condition', () => {
    const rows = [
      row(1, TraceTool.Set, { name: 'R', value: { num: 100, dim: [2, 1, -3, -2, 0, 0, 0] } }),
      row(2, TraceTool.Set, { name: 'R', value: { num: 220, dim: [2, 1, -3, -2, 0, 0, 0] } }),
    ]
  ...
  it('skips a failed set row and a get row', () => {
    const rows = [
      row(1, TraceTool.Set, { name: 'R', value: { num: 1 } }, false),
      row(2, TraceTool.Get, { name: 'R', value: { num: 1 } }),
    ]
    const facts = recordFacts(rows)
