...
纯粹预设中的会话只暴露六个插件工具，没有 shell、文件系统、网络与子代理，因此答案中的每个数字都来自 `set` 的转录或 `eval` 的结果；缺少用户未给出的量时，智能体按人设停下并指明该量。检索预设只多加一个工具 `search`，别的什么也不加：通用的 `web_search` 与 `web_fetch` 工具两个预设都不挂载，所以模型永远无法自行检索或抓取。
