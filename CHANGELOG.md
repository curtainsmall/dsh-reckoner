...
- The `$` notation: 5 constants (`$pi` `$e` `$inf` `$i` `$j`), 19 unary and 4 binary functions, and 6 bounded forms (`$sum`, `$prod` and `$seq` evaluate; `$integral`, `$diff` and `$limit` parse but are refused with `ENGINE_SYMBOL_NOT_EVALUABLE`). Arrays are element-wise, `@x[k]` takes an element and `@th.field` a field, and `->` appears only in `$limit`'s subscript. A position is the brace after `_{` or `^{` — every other `^` is a power — and an exponent must be dimensionless: a real exponent scales the dimension (`(4volt)^2` measures `volt^2`), while a complex exponent needs a dimensionless base, which is what makes a phasor writable as `220*$e^(-$j*$pi/6)`.
