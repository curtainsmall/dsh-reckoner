...
class Parser {
  ...
  private expectPunct(text: string, hint: string): void {
    ...
    fail(EngineErrorCode.InvalidFormula, `${formulaPosition(this.formula, token.at)}: expected "${text}"; found ${tokenLabel(token)}. ${hint}`)
  }
  ...
  private parsePrimary(): Expression {
    ...
    if (token.kind === 'number') {
      ...
      if (scalar === undefined) fail(EngineErrorCode.UnknownError, `internal: the number token "${token.text}" carries no value.`)
